...
//import { getUsuariosByPage} from '@/services/usuariosService';
...
const handleSizeChange = async (val: number) => {
    ...
    emit('paginate', await loadUsuariosByPage(currentPage.value, pageSize.value))
}
...
const loadUsuariosByPage = async (page: number, size: number) => {
    try {
        return await $usuarioService.getUsuariosByPage(page, size);
    } catch (error) {
...
onMounted(async () => {
    emit('paginate', await loadUsuariosByPage(1, pageSize.value))
